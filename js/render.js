'use strict';
// ---------------------------------------------------------------------------
// Canvas renderer. Reads the world, never writes to it.
// ---------------------------------------------------------------------------
...
class Renderer {
  ...
  toScreen(tx, ty) { const s = this.scale; return [(tx - this.cam.x) * s + this.cw / 2, (ty - this.cam.y) * s + this.ch / 2]; }
  ...
  drawGround(tx0, ty0, tx1, ty1, ui) {
    ...
    for (let y = ty0; y <= ty1; y++) for (let x = tx0; x <= tx1; x++) {
      ...
      if (m.plan[i]) {
        ...
        if (own !== 0 && !ui.showEnemyPlans) { if (m.plan[i] === 1) this.drawRoadPlan(sx, sy, s, 'rgba(160,110,70,0.35)'); continue; }
        if (m.plan[i] === 1) this.drawRoadPlan(sx, sy, s, 'rgba(255,230,160,0.9)');
        else {
  ...
  drawRoad(i, x, y, sx, sy, s) {
    const m = this.world.map, ctx = this.ctx, W = m.W;
  ...
  drawField(i, sx, sy, s) {
    const m = this.world.map, ctx = this.ctx;
  ...
  drawOverlay(ui, alpha) {
    ...
    if (ui.tool && h) {
      if (ui.tool.kind === 'build') {
        const def = BUILDINGS[ui.tool.type];
        const bx = h.x - (def.w >> 1), by = h.y - (def.h >> 1) + 1;
        const reason = w.canPlace(0, ui.tool.type, bx, by);
        const ok = !reason;
        if (def.radius) {
        ...
        const spr = this.buildingSprite(ui.tool.type, 0);
        ...
        const d = w.doorOf(def, bx, by);
        const [dx, dy] = this.toScreen(d.x, d.y);
        ...
        ui.placeReason = reason;
      } else if (ui.tool.kind === 'road' || ui.tool.kind === 'field' || ui.tool.kind === 'wine') {
        const kind = ui.tool.kind === 'road' ? 1 : ui.tool.kind === 'field' ? 2 : 3;
        const tiles = ui.drag && ui.drag.tool ? lineTiles(ui.drag.x0, ui.drag.y0, h.x, h.y) : [[h.x, h.y]];
        for (const [x, y] of tiles) {
          if (!m.inb(x, y)) continue;
          const i = m.idx(x, y);
          const ok = kind === 1 ? m.canRoad(i) : m.canField(i);
          const [sx, sy] = this.toScreen(x, y);
          ctx.fillStyle = ok ? (kind === 1 ? 'rgba(255,230,160,0.45)' : kind === 2 ? 'rgba(240,220,90,0.4)' : 'rgba(200,120,220,0.4)') : 'rgba(230,60,50,0.35)';
          ctx.fillRect(sx + 1, sy + 1, s - 2, s - 2);
        }
      } else if (ui.tool.kind === 'demolish') {
        const [sx, sy] = this.toScreen(h.x, h.y);
        const bid = m.bld[m.idx(h.x, h.y)];
      ...
      // hover highlight for buildings
      const b = w.buildings.get(m.inb(h.x, h.y) ? m.bld[m.idx(h.x, h.y)] : 0);
      if (b) {
        const [fx, fy] = this.toScreen(b.x, b.y);
  ...
  drawMinimap(canvas) {
    ...
    const W = m.W, H = m.H;
    if (!this.miniImg || this.miniImg.width !== W) this.miniImg = g.createImageData(W, H);
    const d = this.miniImg.data;
    for (let i = 0; i < m.N; i++) {
      let c = TERRAIN_RGB[m.terrain[i]];
    ...
    for (const bld of w.buildings.values()) {
    ...
    const tmp = this.miniTmp || (this.miniTmp = makeCanvas(W, H));
    tmp.getContext('2d').putImageData(this.miniImg, 0, 0);
    ...
    g.strokeRect(wx0 * sx, wy0 * sy, (wx1 - wx0) * sx, (wy1 - wy0) * sy);
  }
}
