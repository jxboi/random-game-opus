'use strict';
// ---------------------------------------------------------------------------
// Input handling and the side panel. Every control goes through a World.cmd*
// / placement method so the simulation stays the single source of truth.
// ---------------------------------------------------------------------------

const esc = s => String(s).replace(/[&<>"]/g, c => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;' }[c]));
const gIcon = g => `<span class="gi" title="${esc(GOODS[g].name)}">${GOODS[g].icon}</span>`;
...
class UI {
  constructor(game) {
    ...
    this.state = {
      ...
      tab: 'build', placeReason: null, showEnemyPlans: false,
    };
    this.keys = new Set();
    this.messages = [];
    this.lastPanel = '';
  ...
  bindInput() {
    ...
    c.addEventListener('wheel', e => { e.preventDefault(); this.zoomAt(e.offsetX, e.offsetY, e.deltaY < 0 ? 1.12 : 1 / 1.12); }, { passive: false });
  ...
  tileAt(e) {
    const rc = this.el.canvas.getBoundingClientRect();
  ...
  onDown(e) {
    ...
    if (e.button === 1 || (e.button === 2 && !tool)) {
      this.state.drag = { pan: true, sx: e.clientX, sy: e.clientY, cx: this.r.cam.x, cy: this.r.cam.y, moved: false, button: e.button, t };
      return;
    }
    ...
    this.state.drag = { sel: true, sx: e.clientX, sy: e.clientY, cx: this.r.cam.x, cy: this.r.cam.y, moved: false, touch: e.pointerType === 'touch', t, shift: e.shiftKey };
  }
  ...
  onMove(e) {
    ...
    if (e.target !== this.el.canvas && !this.state.drag) return;
    const t = this.tileAt(e);
    this.state.hover = t;
    ...
    if (d.pan || (d.sel && d.touch)) {
      ...
      if (d.moved) {
        this.r.cam.x = d.cx - dx / this.r.scale;
        this.r.cam.y = d.cy - dy / this.r.scale;
      }
    } else if (d.sel) {
      if (Math.hypot(dx, dy) > 6) d.moved = true;
      if (d.moved) {
  ...
  onKey(e, down) {
    ...
    switch (k) {
      ...
      case '+': case '=': this.zoomAt(this.r.cw / 2, this.r.ch / 2, 1.15); break;
      case '-': this.zoomAt(this.r.cw / 2, this.r.ch / 2, 1 / 1.15); break;
    }
    this.refreshPanel(true);
  }

  // Called every frame for held keys.
  update(dt) {
    if (this.game.overlayOpen) return;
    const sp = 22 * dt / this.r.cam.zoom;
    const k = this.keys;
  ...
  zoomAt(sx, sy, f) {
    ...
    this.r.cam.zoom = clamp(this.r.cam.zoom * f, 0.45, 2.2);
    const [nx, ny] = this.r.toWorld(sx, sy);
    this.r.cam.x += wx - nx; this.r.cam.y += wy - ny;
  }
  ...
  setTool(tool) {
    ...
    this.el.canvas.style.cursor = tool ? (tool.kind === 'demolish' ? 'not-allowed' : 'crosshair') : 'default';
    this.refreshPanel(true);
  }
  ...
  placeAt(t) {
    ...
    this.world.placeBuilding(0, type, bx, by, false);
    if (!this.game.shiftHeld()) this.setTool(null);
  }

  demolishAt(t) {
    const what = this.world.demolish(0, t.x, t.y);
  ...
  joinAt(t) {
    ...
    if (u && u.group && u.group !== tool.gid) {
      this.world.cmdJoin(tool.gid, u.group);
  ...
  unitAt(t, owner, soldiersOnly) {
    ...
    for (const u of this.world.units.values()) {
      ...
      const d = (u.x - t.wx) ** 2 + (u.y - 0.25 - t.wy) ** 2;
  ...
  ownThingAt(t) {
    ...
    const b = m.inb(t.x, t.y) && this.world.buildings.get(m.bld[m.idx(t.x, t.y)]);
    return !!(b && b.owner === 0);
  }
  ...
  clickSelect(t, add) {
    ...
    const b = m.inb(t.x, t.y) ? this.world.buildings.get(m.bld[m.idx(t.x, t.y)]) : null;
    this.clearSelection();
  ...
  boxSelect(box) {
    const [ax, ay] = this.r.toWorld(Math.min(box.x0, box.x1), Math.min(box.y0, box.y1));
    const [bx, by] = this.r.toWorld(Math.max(box.x0, box.x1), Math.max(box.y0, box.y1));
    const sel = new Set();
    for (const u of this.world.units.values()) {
      if (u.owner !== 0 || u.kind !== 'soldier' || u.inside) continue;
      if (u.x >= ax && u.x <= bx && u.y >= ay && u.y <= by) sel.add(u.group);
    }
    this.clearSelection();
  ...
  orderAt(x, y, t) {
    ...
    const bld = m.inb(x, y) ? w.buildings.get(m.bld[m.idx(x, y)]) : null;
    if (enemy) {
      for (const g of groups) w.cmdAttack(g, enemy.id);
      this.r.flash(enemy.x, enemy.y, 'rgba(255,80,60,A)');
      this.game.sfx.tone(300, 0.08, 'square', 0.12);
    } else if (bld && bld.owner !== 0) {
      for (const g of groups) w.cmdAttackBuilding(g, bld.id);
      ...
      groups.forEach((g, k) => {
        ...
        const off = (k - (n - 1) / 2) * 4;
        w.cmdMove(g, clamp(Math.round(x - fy * off), 0, m.W - 1), clamp(Math.round(y + fx * off), 0, m.H - 1));
      });
  ...
  cmdGroups(cmd, arg) {
    ...
    for (const gid of [...this.state.selGroups]) {
      switch (cmd) {
        case 'halt': w.cmdHalt(gid); break;
        case 'split': { const ng = w.cmdSplit(gid); if (ng) this.state.selGroups.add(ng.id); break; }
        case 'form': w.cmdFormation(gid, arg); break;
        case 'turn': w.cmdTurn(gid, arg); break;
        case 'feed': w.cmdFeed(gid); break;
      }
    }
    if (cmd === 'join') {
      const gid = [...this.state.selGroups][0];
      if (gid) { this.state.tool = { kind: 'join', gid }; this.el.canvas.style.cursor = 'copy'; this.toast('Click a group to join'); }
    }
    this.game.sfx.click();
  }
  ...
  onPanelClick(e) {
    ...
    switch (a) {
      ...
      case 'school': if (w.cmdSchoolQueue(bid, v)) this.game.sfx.click(); else this.game.sfx.error(); break;
      case 'school-cancel': w.cmdSchoolCancel(bid, +v); this.game.sfx.click(); break;
      case 'train': {
        const n = e.shiftKey ? 5 : 1;
        if (w.cmdBarracksTrain(bid, v, n)) this.game.sfx.click();
        break;
      }
      case 'train-cancel': w.cmdBarracksCancel(bid, +v); break;
      case 'order': w.cmdToggleOrder(bid, +v); this.game.sfx.click(); break;
      case 'block': w.cmdToggleBlock(bid, v); this.game.sfx.click(); break;
      case 'demolish-b': {
        const b = w.buildings.get(bid);
        if (b && b.owner === 0) { w.destroyBuilding(b, 'demolish'); this.clearSelection(); this.game.sfx.place(); }
        break;
      }
  ...
  tick(dt) {
    ...
    if (this.miniT <= 0) { this.miniT = 0.4; this.r.drawMinimap(this.el.mini); }
  }
  ...
  refreshPanel(force) {
    ...
    const tipText = this.state.tool && this.state.tool.kind === 'build' && this.state.placeReason && this.state.hover ? '⚠ ' + this.state.placeReason : this.hoverInfo;
