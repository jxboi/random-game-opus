...
function roadPathToNetwork(w, owner, start, blocked) {
  const m = w.map, W = m.W;
...
function autoPlace(w, owner, type, cx, cy, instant) {
...
function layFields(w, b, n, instant) {
  const m = w.map;
  const cand = [];
...
function spawnGroup(w, owner, type, n, ax, ay, fx, fy) {
  if (n <= 0) return null;
...
const PLAYER_TOWN = ['school', 'inn'];
const AI_TOWN = ['school', 'inn', 'barracks', 'sawmill', 'woodcutter', 'quarry', 'farm', 'mill', 'bakery',
  'weaponshop', 'armorshop', 'woodcutter', 'swine', 'butcher'];

function setupTowns(w, opts = {}) {
  const m = w.map;
