...
import { BUILDINGS, DIFFICULTY } from './data.js';
...
export const MAP_ARRAYS = ['terrain', 'ore', 'tree', 'treeT', 'stone', 'fish', 'road', 'roadOwner', 'field', 'fieldStage',
  'fieldT', 'fieldOwner', 'plan', 'planOwner', 'planJob', 'bld', 'reserve', 'height'];

export function b64enc(ta) {
  const u8 = new Uint8Array(ta.buffer, ta.byteOffset, ta.byteLength);
  let s = '';
  for (let i = 0; i < u8.length; i += 0x8000) s += String.fromCharCode.apply(null, u8.subarray(i, i + 0x8000));
  return btoa(s);
}
export function b64dec(str, Type) {
  const s = atob(str);
...
export function saveWorld(w) {
  ...
  const ai = w.ai ? {
    nextAttack: w.ai.nextAttack, nextTrickle: w.ai.nextTrickle, nextRebuild: w.ai.nextRebuild,
    waves: w.ai.waves, threatT: w.ai.threatT, threat: w.ai.threat, home: w.ai.home, layout: w.ai.layout,
  } : null;
  // Infinity doesn't survive JSON; store it as null.
  if (ai && !Number.isFinite(ai.nextAttack)) ai.nextAttack = null;
  return {
    v: SAVE_VERSION, seed: w.seed, difficulty: w.difficulty, time: w.time, tickN: w.tickN,
    nextId: w.nextId, rng: w.rng.s, over: w.over, map,
    players: w.players.map(p => ({ id: p.id, isAI: p.isAI, defeated: p.defeated, stats: p.stats,
      lastAttackAlert: p.lastAttackAlert, lastHungryAlert: p.lastHungryAlert, lastIdleAlert: p.lastIdleAlert })),
    units: [...w.units.values()],
    buildings: [...w.buildings.values()].map(b => { const o = Object.assign({}, b); delete o.def; return o; }),
    groups: [...w.groups.values()],
    cjobs: [...w.cjobs.values()],
    projectiles: w.projectiles, effects: w.effects,
    fieldTiles: [...w.fieldTiles],
    ai,
  };
}
