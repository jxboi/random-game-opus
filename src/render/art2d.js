...
import { FIELD_GROW_TIME, T_GRASS, T_MOUNTAIN, T_SAND, T_WATER, WINE_GROW_TIME } from '../core/map.js';
import { RNG, clamp, hash2, smoothstep, valueNoise } from '../core/util.js';
import { TILE } from './common.js';

export const SPR = 64; // sprite resolution (px per tile)
...
export const ORE_RGB = [null, [34, 30, 28], [176, 96, 58], [242, 200, 56]];

// relief scales the baked hill shading: 1 for the flat 2D view, lower for 3D
// where the terrain mesh is lit for real.
export function renderTerrainChunk(map, cx, cy, seed, relief = 1) {
  ...
  for (let py = 0; py < size; py++) {
    ...
    for (let px = 0; px < size; px++) {
      ...
      let sh = 1 + (-dhdx * 0.45 - dhdy * 0.6) * kk * relief;
...
// ------------------------------------------------------- ground overlays
// Shared by the 2D renderer (per frame) and the 3D terrain textures (on change).
export function paintRoadPlan(ctx, sx, sy, s, col) {
  ctx.fillStyle = col;
  for (let k = 0; k < 3; k++) {
    ctx.beginPath(); ctx.arc(sx + s * (0.25 + k * 0.25), sy + s * 0.5, s * 0.06, 0, 7); ctx.fill();
  }
}


export function paintRoad(ctx, m, i, x, y, sx, sy, s) {
  const W = m.W;
  const own = m.roadOwner[i];
  const conn = k => m.road[k] && m.roadOwner[k] === own;
  const n = y > 0 && conn(i - W), so = y < m.H - 1 && conn(i + W), e = x < W - 1 && conn(i + 1), wv = x > 0 && conn(i - 1);
  const inset = 0.16;
  ctx.fillStyle = '#9a7b52';
  const a = inset * s;
  ctx.fillRect(sx + a, sy + a, s - 2 * a, s - 2 * a);
  if (n) ctx.fillRect(sx + a, sy, s - 2 * a, a);
  if (so) ctx.fillRect(sx + a, sy + s - a, s - 2 * a, a + 0.5);
  if (wv) ctx.fillRect(sx, sy + a, a, s - 2 * a);
  if (e) ctx.fillRect(sx + s - a, sy + a, a + 0.5, s - 2 * a);
  if (s > 18) { // cobbles
    ctx.fillStyle = 'rgba(70,50,30,0.35)';
    for (let k = 0; k < 4; k++) {
      const px = hash2(x, y, 20 + k), py = hash2(x, y, 30 + k);
      ctx.fillRect(sx + s * (0.2 + px * 0.55), sy + s * (0.2 + py * 0.55), s * 0.09, s * 0.07);
    }
    ctx.fillStyle = 'rgba(255,240,200,0.12)';
    ctx.fillRect(sx + a, sy + a, s - 2 * a, s * 0.04);
  }
}


export function paintField(ctx, m, i, sx, sy, s) {
  const kind = m.field[i], st = m.fieldStage[i];
  ctx.fillStyle = kind === 1 ? '#6e5234' : '#5c4a30';
  ctx.fillRect(sx + 1, sy + 1, s - 2, s - 2);
  const grow = st === 2 ? 1 : st === 1 ? m.fieldT[i] / (kind === 1 ? FIELD_GROW_TIME : WINE_GROW_TIME) : 0;
  ctx.lineWidth = Math.max(1, s * 0.05);
  if (kind === 1) {
    ctx.strokeStyle = 'rgba(40,28,16,0.5)';
    for (let r = 0; r < 4; r++) { const yy = sy + s * (0.18 + r * 0.22); ctx.beginPath(); ctx.moveTo(sx + 2, yy); ctx.lineTo(sx + s - 2, yy); ctx.stroke(); }
    if (st > 0) {
      ctx.strokeStyle = st === 2 ? '#e8c448' : `rgb(${90 + grow * 90},${150 + grow * 40},${50})`;
      const h = s * (0.06 + grow * 0.16);
      for (let r = 0; r < 4; r++) for (let c = 0; c < 5; c++) {
        const xx = sx + s * (0.12 + c * 0.19), yy = sy + s * (0.18 + r * 0.22);
        ctx.beginPath(); ctx.moveTo(xx, yy); ctx.lineTo(xx + s * 0.02, yy - h); ctx.stroke();
      }
    }
  } else {
    for (let c = 0; c < 3; c++) {
      const xx = sx + s * (0.2 + c * 0.3);
      ctx.strokeStyle = '#4a3420'; ctx.beginPath(); ctx.moveTo(xx, sy + s * 0.1); ctx.lineTo(xx, sy + s * 0.9); ctx.stroke();
      if (st > 0) {
        ctx.fillStyle = `rgba(70,${110 + grow * 40},50,0.95)`;
        for (let k = 0; k < 4; k++) { ctx.beginPath(); ctx.arc(xx + (k % 2 ? 0.07 : -0.07) * s, sy + s * (0.2 + k * 0.2), s * (0.05 + grow * 0.05), 0, 7); ctx.fill(); }
        if (st === 2) { ctx.fillStyle = '#6a2a6a'; for (let k = 0; k < 3; k++) { ctx.beginPath(); ctx.arc(xx + s * 0.05, sy + s * (0.3 + k * 0.22), s * 0.05, 0, 7); ctx.fill(); } }
      }
    }
  }
}
...
// Road / field plan marker. Enemy plans are drawn faintly (roads only).
export function paintPlan(ctx, m, i, sx, sy, s, showEnemy = false) {
  const own = m.planOwner[i];
  if (own !== 0 && !showEnemy) { if (m.plan[i] === 1) paintRoadPlan(ctx, sx, sy, s, 'rgba(160,110,70,0.35)'); return; }
  if (m.plan[i] === 1) { paintRoadPlan(ctx, sx, sy, s, 'rgba(255,230,160,0.9)'); return; }
  ctx.strokeStyle = m.plan[i] === 2 ? 'rgba(240,220,90,0.9)' : 'rgba(200,120,220,0.9)';
  ctx.setLineDash([s * 0.12, s * 0.1]); ctx.lineWidth = Math.max(1, s * 0.05);
  ctx.strokeRect(sx + s * 0.12, sy + s * 0.12, s * 0.76, s * 0.76);
  ctx.setLineDash([]);
}

// Everything drawn flat on a ground tile (fields, roads, plans), in paint order.
export function paintTileOverlay(ctx, m, i, x, y, sx, sy, s, showEnemy = false) {
  if (m.field[i]) paintField(ctx, m, i, sx, sy, s);
  if (m.road[i]) paintRoad(ctx, m, i, x, y, sx, sy, s);
  if (m.plan[i]) paintPlan(ctx, m, i, sx, sy, s, showEnemy);
}

// -------------------------------------------------------------- trees/stone
export function makeTreeSprite(stage, variant) {
...
export function makeStoneSprite(size, variant) {
...
export const ROOF = { tile: ['#a4442c', '#7a2f1e'], thatch: ['#c9a257', '#9b7a3a'], slate: ['#5d6673', '#434a55'] };
export const TOP = 1.4; // sprite margin above the footprint (tiles) for roofs and towers

// Draws a building sprite. Coordinates are in tiles; (0, TOP) is the footprint's top-left.
export function makeBuildingSprite(type, color) {
...
export function roundTop(g, x, y, w, h) {
  g.beginPath();
...
export function flag(g, x, y, color, h) {
...
export function drawWall(g, kind, x, y, w, h) {
  if (kind === 'stone') {
...
export function drawProps(g, props, W, Hh, top) {
...
export function drawYard(g, bw, W, top, Hh, kind) {
...
export function drawMine(g, W, Hh, top, type, color) {
...
export function drawQuarry(g, W, Hh, top, color) {
  const yf = top + Hh - 0.1;
  // lean-to shed
...
export function crenels(g, x, y, w, color) {
  g.fillStyle = color;
  for (let xx = x; xx < x + w - 0.05; xx += 0.2) g.fillRect(xx, y - 0.12, 0.12, 0.12);
}

export function drawTower(g, W, Hh, top, color) {
...
export function drawBarracks(g, W, Hh, top, color) {
  const yf = top + Hh - 0.1;
  // courtyard wall
...
export function drawMill(g, W, Hh, top, color) {
...
export function buildingIcon(type, color, size = 44) {
